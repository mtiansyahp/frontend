...
const PatternCategory = () => {
  ...
  function handleEdit(id) {
    ...
    setIsUpdate(true);
    setIdUpdate(id);
    let data = [...patterncategory];
    let foundData = data.find((category) => category.id === id);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isUpdate) {
      await axios
        .post(base_url + "/texture-category/add-texture-category", newData, {
          headers: {
      ...
      await axios
        .put(
          base_url + "/texture-category/update-texture-category/" + idUpdate,
          newData
        )
        .then((response) => {
          setLoading(false)
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await axios.delete(
            base_url + "/texture-category/delete-texture-category/" + id
          ).then(response => {
            if (response.data.success) {
              setLoading(false)
              setIsOpenForm(false);
  ...
  const getPatternCategory = async (e) => {
    const response = await axios.get(
      `${base_url}/texture-category/get-texture-category?search_query=${keyword}&page=${page}&limit=${limit}`
    );

    setPatterncategory(response.data.data.result);
    ...
    >
      ...
      <div className="overflow-x-auto scrollbar-custom bg-white overflow-hidden mb-3">
        <table className="sm:w-full w-max border border-black">
          ...
          <tbody>
            {patterncategory.map((category, index) => (
              <tr
